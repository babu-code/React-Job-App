...
const JobPage = ({deleteJob}) => {
    ...
    let jobCompanyDescription =job.company.description
    
    if (!showMore){
      jobCompanyDescription = jobCompanyDescription.substring(0,100) + '...'
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
          ...
          <aside>
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <p className="my-2">{jobCompanyDescription} <Link to="" onClick={()=>(setShowMore((prevState)=>(!prevState)))} className='text-indigo-500'>{showMore ? 'less' : 'More'}</Link> </p>
